...
export const geSkillTotalDps = (
  ...
  // 计算身法带来的面板增益
  const 强膂郭氏身法 = 最终人物属性?.强膂 ? 102 : 0
  // 郭氏身法在是否开启强膂下的提升百分比
  const guoLidaoPercent =
    (1024 + 计算郭氏身法 + 强膂郭氏身法) / 1024 / ((1024 + 强膂郭氏身法) / 1024) - 1
  // 郭式身法对人物属性身法的提升值
  const 郭式身法对人物属性身法的提升值 = 最终人物属性.身法 * guoLidaoPercent
  // 身法数值的提升值
  const 身法提升值 = 计算身法加成 + (计算身法加成 * (计算郭氏身法 + 强膂郭氏身法)) / 1024
  ...
  最终人物属性 = {
    ...最终人物属性,
    身法: 最终人物属性.身法 + 总身法提升值,
    基础攻击: 最终人物属性.基础攻击 + 身法提升值 * 加成系数.身法加成基础攻击,
    面板攻击: (最终人物属性.面板攻击 =
      最终人物属性.面板攻击 +
      身法提升值 * 加成系数.身法加成面板攻击 +
      身法提升值 * 加成系数.身法加成基础攻击),
    会心值: (最终人物属性.会心值 = 最终人物属性.会心值 + 身法提升值 * 加成系数.身法加成会心),
    破防值: (最终人物属性.破防值 = 最终人物属性.破防值 + 身法提升值 * 加成系数.身法加成破防),
  }
