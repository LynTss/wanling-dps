...
export const geSkillTotalDps = (
  ...
  // 郭式身法对人物属性身法的提升值
  const 郭式身法对人物属性身法的提升值 = 增益计算基础?.最终人物属性.身法 * guoShenfaPercent
  // 身法数值的提升值
  const 身法提升值 =
    增益计算基础?.身法数值加成 +
    (增益计算基础?.身法数值加成 * (增益计算基础?.郭氏身法 + 卢令郭氏身法)) / 1024
  ...
  增益计算基础 = {
    ...
    最终人物属性: {
      ...
      面板攻击: 增益计算基础?.最终人物属性.面板攻击 + 身法提升值 * 加成系数.身法加成面板攻击,
      会心值: 增益计算基础?.最终人物属性.会心值 + 身法提升值 * 加成系数.身法加成会心,
    },
  }
