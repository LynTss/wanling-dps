...
export const 修改装备信息 = (原装备信息, 当前附魔数据) => {
  return {
    ...
    装备列表: 原装备信息.装备列表?.map((item, index) => {
      ...
      const 是否符合部位 = item.装备部位 === EquipmentCharacterPositionEnum[当前索引]
...
export const 初始化所有组合 = () => {
  ...
  Object.keys(EquipmentCharacterPositionEnum).forEach((key) => {
    AllEnchantDTO.forEach((item) => {
      if (item?.附魔支持部位?.includes(EquipmentCharacterPositionEnum[key] as any)) {
